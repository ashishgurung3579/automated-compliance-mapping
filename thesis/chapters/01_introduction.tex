...
\begin{enumerate}[label=\textbf{RQ\arabic*.},leftmargin=3.2em]
  ...
  \item How do different automated approaches --- lexical baselines (TF--IDF,
  security-keyword Jaccard), sentence-embedding models (Sentence-BERT, MPNet,
  BGE), contextual-embedding models (BERT, SecureBERT, CySecBERT), a
  cross-encoder trained for natural language inference, and hosted API methods
  (Gemini embeddings and Gemini prompt classification) --- compare on the
  compliance mapping task?
...
\begin{itemize}
  ...
  classification --- with a common evaluation harness.
  ...
  these two standards --- 13 pairs among 1{,}027 judged, and none among 300
  top-ranked pairs searched specifically to find more --- which bounds what any
  typed mapping between a device-level and a lifecycle-level standard can report.
\end{itemize}
