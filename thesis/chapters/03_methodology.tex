...
score --- the mean percentile rank of each pair across every similarity matrix
--- and 650 were drawn from three bands of that ordering, 250 from the top tenth,
200 from the next three tenths and 200 from the bottom six tenths. Averaging over
the methods keeps the screen neutral between them; screening on one model's
scores would sample densely wherever that model is confident and quietly flatter
it at evaluation. Batches were shuffled under a fixed seed, and the 107 pilot
...
judged --- ranked by the same method-agnostic screen, so no evaluated method is
privileged --- were annotated in eight further batches.

That round returned \emph{no} equivalence or subsumption pairs at all: 184
\textsc{No relation}, 65 \textsc{Overlap}, 51 \textsc{Complementarity}. The
result is worth stating as a finding rather than a shortfall. Among the pairs
already judged, the rare classes occur at roughly one in 250 even in the
...
organizations around it; a strict-superset or same-scope relation rarely survives
that difference in layer. For this pair of standards the typed taxonomy is
effectively a three-class problem.
...
has and the remainder is redistributed, which is what the scarcity of the rare
classes forces here.
...
One limitation follows directly from the rule and is stated here rather than
left to the reader. A positive rate of one half is a design choice, not a
property of the corpus, and precision and F1 both move with it. Every figure
reported in Chapter~\ref{ch:results} therefore describes performance on a
balanced set and is an upper bound on what the same operating point achieves
across the full 4{,}968 candidates, which are far sparser. Quantifying that gap
would require a sample drawn with known inclusion probabilities; this design does
not have one, and Section~\ref{sec:futurework} names it as the first extension
the work should receive.
...
exactly the ones a compliance analyst cares most about --- but with three and ten
supporting pairs respectively, those two components move substantially on a
single prediction, and the macro average inherits that instability. The
...
intervals will contain false findings whatever the data say. Two questions carry the argument --- whether a
