...
framework to those of another, so that proofs collected for one can be
reused for the other and gaps become noticeable. Studies of security standards
...
LLMs can be prompted to evaluate two requirements directly and output a
...
is the goal a control is intended to realise rather than by the words it
uses. The authors state a deliberate preference for under mapping, over
over-mapping when a match is uncertain. Their taxonomy is binary which shows two
...
documents are processed into a uniform XML structure, requirement entities like 
...
showcase that ontology supported NLP can automate requirement mapping.
however, that ambiguity and implicit, domain specific meaning remain the
hard part in unclear document. The same difficulties appear with the standards studied here.

\subsection{LLM-based compliance analysis}

Rodríguez et al.~\cite{rodriguez2024llm} investigate LLMs for large-scale
privacy policy analysis, comparing ChatGPT and Llama2 against earlier
...
surpassing prior techniques while requiring less annotated data and
engineering effort. The study is constrained to privacy policies, but it provides
strong proof that prompted LLMs can support compliance oriented text
