...
% NOTE: confirm with supervisor. The approved degree project plan
% (docs/Compliance_Thesis_Project_2026.pdf) says 60 Higher Education credits,
% but its metadata block is largely unfilled template text — the keywords field
% still reads "Word1, Word2, ..." — so 60 may be the IV-Master default rather
% than a deliberate figure. If it is 60, the scope justifications in
% 03_methodology.tex (sec:gt) and 06_conclusion.tex must change too: both
% currently excuse the LLM-assisted reference set as a necessity "at 15
% credits", which does not hold for a full-year project.
