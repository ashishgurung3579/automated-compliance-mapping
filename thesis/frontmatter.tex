...
them to each other with seven methods: TF--IDF similarity, security-keyword
matching, three transformer embedding models (Sentence-BERT, BERT, SecureBERT),
Gemini text embeddings, and a Gemini LLM prompted to classify each of the 4{,}968
possible pairs into one of six relationship types. Predictions were evaluated
...
12.4\%, and at that rate four of the seven methods score exactly the all-positive
baseline, rejecting almost no unrelated pair. Most of that collapse is
calibration rather than capability: re-selecting each threshold by
cross-validation on the sample lifts Gemini embeddings to F1 0.50 and
Sentence-BERT to 0.42 against a baseline of 0.22, differences a paired bootstrap
separates from zero, while SecureBERT clears the baseline at no threshold at all.
