...
corpus-calibrated threshold, and that method's precision is far below one. The
figures and tables written by this module therefore carry the method's measured
precision and its confidence interval in their captions, and nothing here should
be read as a verified mapping. The verified statements in this thesis are the
ones estimated from the probability sample.
...
def reference_method(weighted: dict) -> tuple[str, float, dict]:
    """The method the coverage map is built from, and the precision that qualifies it.

    Chosen as the highest corpus F1 at a corpus-calibrated operating point, which
    is the same reference the paired comparisons use.
    """
    name = weighted["paired_reference"].removesuffix("@corpus")
    entry = weighted["methods"][name]
    calibrated = entry.get("corpus_calibrated")
    if calibrated is None:
        raise SystemExit(f"reference method {name} has no corpus-calibrated point")
    return name, calibrated["threshold"], calibrated
...
if __name__ == "__main__":
    ...
    weighted = json.loads((EVAL_DIR / "weighted_metrics.json").read_text())
    name, threshold, calibrated = reference_method(weighted)
    method = METHODS[name]
    precision = calibrated["point"]["precision"]
    ci = calibrated["ci"]["precision"]
    ...
    print(f"  corpus-calibrated threshold {threshold:.3f}, "
          f"precision {precision:.3f} {ci}")
