...
if __name__ == "__main__":
    ...
    print(f"Encoding with {MODEL}  (downloads ~500 MB on first run)...")
    src_emb = embed_with_transformers(MODEL, src_texts, use_safetensors=False)
    tgt_emb = embed_with_transformers(MODEL, tgt_texts, use_safetensors=False)
        ...
        for j, tp in enumerate(tgt):
            ...
            if score >= THRESHOLD:
                rows.append({
                    ...
                    "securebert_score": round(score, 4),
                    "predicted_rel": score_to_relation_semantic(score),
                })

    df = pd.DataFrame(rows).sort_values("securebert_score", ascending=False)
    df.to_csv(out_dir / "securebert_output.csv", index=False)
    print(f"Candidates (threshold={THRESHOLD}): {len(df)}")
    print(df["predicted_rel"].value_counts().to_dict())
    print(f"Saved: {out_dir / 'securebert_output.csv'}")
