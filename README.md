...
a three-class problem. This is a finding about the standards, not a sampling shortfall.
...
Every method, with its model, family, threshold, and output filenames, is defined once in
...
Stage 6's output is computed but not reported in the thesis. The judged fraction of each ranking is
...
round, 300). The confidence scale is defined 1–3 throughout. The screened and targeted rounds
used only levels 2 and 3, so the calibration result is a two-level contrast rather than a
curve; level 1 appears five times in the pilot, three of which reach the reference set.
...
Stage 5 also reports each method with a stored score matrix at two operating points: the threshold
fixed per family before evaluation, and one re-selected by repeated stratified cross-validation over
quantiles of the method's own scores. The second point is needed because several methods ship at a
threshold that places every pair on one side of the line. Method orderings are claimed only where a
paired bootstrap on the difference excludes zero.
